...
public final class SaveTiles {
	...
	public static String saveLayers(HashMap<Layer, Array<Float>> tiles) {
		if (tiles == null) return "";
		StringBuilder sb = new StringBuilder("{");

		for (Map.Entry<Layer, Array<Float>> entry : tiles.entrySet()) {
	...
	public static HashMap<Layer, Array<Float>> loadLayers(String tiles) {
		if (tiles == null || tiles.isEmpty() || Utility.isBlank(tiles)) return null;
		HashMap<Layer, Array<Float>> map = new HashMap<>();
		...
		for (int i = 1; i < tiles.length() - 1; i++) {
			...
			map.put(Utility.stringToEnum(name.toString(), Layer.values()), tilesArray);
		}
