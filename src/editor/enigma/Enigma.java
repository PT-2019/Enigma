...
public class Enigma implements ActionListener {
    ...
    public Enigma(Map<String,Object> attributes){
        ...
        if(attributes.containsKey(Attributes.DESCRIPTION)) this.description = (String) attributes.get(Attributes.DESCRIPTION);
        else throw new IllegalArgumentException("Attribut \"description\" abscent");
        if(attributes.containsKey(Attributes.KNOWN)) this.known = Boolean.parseBoolean((String) attributes.get(Attributes.KNOWN));
        else throw new IllegalArgumentException("Attribut \"known\" abscent");
        if(attributes.containsKey(Attributes.CURRENT_ADVICE_INDEX)) this.currentAdvice = Integer.parseInt((String) attributes.get(Attributes.CURRENT_ADVICE_INDEX));
        else throw new IllegalArgumentException("Attribut \"currentAdviceIndex\" abscent");
        if(attributes.containsKey(Attributes.ADVICES)) this.advices = (ArrayList<Advice>) attributes.get(Attributes.ADVICES);
        else throw new IllegalArgumentException("Attribut \"advices\" abscent");
        if(attributes.containsKey(Attributes.CONDITIONS)) this.conditions = (ArrayList<Condition>) attributes.get(Attributes.CONDITIONS);
        else throw new IllegalArgumentException("Attribut \"conditions\" abscent");
        if(attributes.containsKey(Attributes.OPERATIONS)) this.operations =  (ArrayList<Operation>) attributes.get(Attributes.OPERATIONS);
