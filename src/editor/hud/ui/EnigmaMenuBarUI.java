...
public class EnigmaMenuBarUI extends BasicMenuBarUI {
	...
	public void paint(Graphics g, JComponent c) {
		...
		if (this.border != null) {
			...
			for (int i = 0; i < 4; i++) {
				if (i == EnigmaUIValues.TOP_BORDER && this.showedBorders[i])
					brush.fillRect(0, 0, mb.getWidth(), this.borderSize);
				if (i == EnigmaUIValues.RIGHT_BORDER && this.showedBorders[i])
					brush.fillRect(mb.getWidth() - this.borderSize, 0, mb.getWidth(), mb.getHeight());
				if (i == EnigmaUIValues.BOTTOM_BORDER && this.showedBorders[i])
					brush.fillRect(0, mb.getHeight() - this.borderSize, mb.getWidth(), mb.getHeight());
				if (i == EnigmaUIValues.LEFT_BORDER && this.showedBorders[i])
