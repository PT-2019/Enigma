...
public class JsonFile implements Serializable {
	...
	private static final class JsonSubTexture {
		...
		private static Image getSubTexture(final String[] jsonFile, final String image, Integer bounds){
			...
			BufferedImage img;//TODO: put this in a method (hide getSubImage)
			try{
				//open file
				FileInputStream input = new FileInputStream(new File(image));
				//read image
				img = ImageIO.read(input);
				//close file
				input.close();
			} catch (IOException e){
				throw new IllegalStateException("...un message...");
			}

			return img.getSubimage(pos.x,pos.y,size.width, size.height);
		}
