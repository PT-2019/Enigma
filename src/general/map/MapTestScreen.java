...
import general.entities.special.GameExit;
import general.entities.types.ContainersManager;
import general.entities.types.NeedContainer;
import general.entities.types.NeedContainerManager;
import general.save.TmxProperties;
...
public class MapTestScreen extends AbstractMap {
	...
	private void setFromSave(GameObject entity, Vector2 start) {
		boolean noForce = (entity instanceof general.entities.types.Container);
		...
		if (start != null) {
			...
			if (needContainer || needManager) {
				...
				for (Map.Entry<Vector2, GameObject> entries : parentObject.entrySet()) {
					//ok
					if (needContainer && entries.getValue() instanceof general.entities.types.Container)
						break;
					//ok
